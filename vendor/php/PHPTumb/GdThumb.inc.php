...
class GdThumb extends ThumbBase
{
	...
	public function __construct ($fileName, $options = array(), $isDataStream = false)
	{
		...
		$this->initialDimensions = $this->currentDimensions;
	...
	public function adaptiveResize ($width, $height)
	{
		...
		if($cropX == 0){
			// crop vertical
			// originial taller than resize
			
			$ratio = $this->initialDimensions['width'] / $this->maxWidth; // maxWidth -- check for bigger images?
			
			$csy   = $ratio * $cropY;
			$ch    = $ratio * $this->maxHeight;
			
			$cw  = $this->initialDimensions['width'];
			$csx = 0;
			}
		elseif($cropY == 0){
			// crop orizontal
			// original wider than resize
			
			$ratio = $this->initialDimensions['height'] / $this->maxHeight; // maxHeight -- check for bigger images?
			$csx   = $ratio * $cropX;
			$cw    = $ratio * $this->maxWidth;
			
			
			$ch  = $this->initialDimensions['height'];
			$csy = 0;
			}
	 ...
	 * @param int $height
	 */
	public function calcImageSizeStrict ($width, $height)
	{
