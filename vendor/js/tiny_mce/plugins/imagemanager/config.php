...
if(!defined('BOOTSTRAP_LOADED')){
	...
	$tmp = array_slice($tmp, 0, count($tmp)-5);
	$tmp = implode($ds, $tmp);
    // TODO: require($tmp.$ds."!bootstrap.php");
	//
	}
	...
	$mcImageManagerConfig['preview.urlprefix'] = "{proto}://{host}".WWW_ROOT; // domain name
	...
	$mcImageManagerConfig['filesystem.path'] = FILE_ROOT; // absolute or relative from this script path, optional.
	$mcImageManagerConfig['filesystem.rootpath'] = FILE_ROOT."uploads".DS."editor".DS; // absolute or relative from this script path, required.
	...
	$mcImageManagerConfig['upload.autoresize_jpeg_quality'] = 70; // Force max width/height, IM will rescale uploaded images.
	$mcImageManagerConfig['upload.max_width'] = "658"; // Only if force_width_height is true
	$mcImageManagerConfig['upload.max_height'] = "3000"; // Only if force_width_height is true
